...
const seedAgents = async () => {
  ...
  for (const agent of agents) {
    await db.run(
      `INSERT OR IGNORE INTO agents (name, type, capabilities, config) VALUES (?, ?, ?, ?)`,
...
const seedDirector = async () => {
  ...
  await db.run(
    `INSERT OR IGNORE INTO directors (username, display_name, email, preferences, config) VALUES (?, ?, ?, ?, ?)`,
...
module.exports = { migrate, createTables, seedAgents, seedDirector };